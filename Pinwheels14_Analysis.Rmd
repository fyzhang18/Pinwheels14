...
orig.sample <- read.csv("/Volumes/emberson/ResearchProjects/Pupillometry/Pinwheels14/Pinwheel14_Infant_Preprocessed_RemovedOutliers.csv") 
orig.sample$X <- NULL
...
moo <- data.frame(subID=numeric(18),trialtype=numeric(18),percent=numeric(18))
moo$subID <- subs
moo$trialtype <- rep(1:3, each = 6)
...
if (length(z)==0) { #if doesn't match
  ...
  moo$percent[j+6] <- v.incor2$numoftrials[z] / v2$numoftrials[zz]
}
} 
...
if (length(z)==0) { #if doesn't match
  ...
  moo$percent[j+12] <- v.no2$numoftrials[z] / v2$numoftrials[zz]
}} 

#plot
moo$trialtype[1:6] <- 3
moo$trialtype[13:18] <- 1
...
#create empty dataframe
subs <- unique(orig.sample5$subID)
goo <- data.frame(subID=numeric(30),trialtype=numeric(30),percent=numeric(30))
goo$subID <- subs
goo$trialtype <- rep(1:3, each = 10)
...
if (length(z)==0) { #if doesn't match
  ...
  goo$percent[j+10] <- w.mismatch2$numoftrials[z] / w2$numoftrials[zz]
}
} 
...
if (length(z)==0) { #if doesn't match
  ...
  goo$percent[j+20] <- w.no2$numoftrials[z] / w2$numoftrials[zz]
}} 

#plot
goo$trialtype[1:10] <- 3
goo$trialtype[20:30] <- 1
...
#preswitch only
koofinal <- group_by(koo2, switch, trialtype.aem) %>%
  summarise(
    meanTRIALS=mean(numoftrials,na.rm = TRUE),
    seTRIALS=sd(numoftrials, na.rm = TRUE)/sqrt(length(numoftrials))
  )
