...
poo <- subset(foo, TRIAL_INDEX < 49) #48 trials (3*16)
...
length(which(foo$RIGHT_PUPIL_SIZE=="."))
...
#3. Standardize time 
baseline <- poo %>%
  group_by(subID, TRIAL_INDEX) %>%
  summarise(starttime = min(TIMESTAMP))
# combine to data base (this step takes long bc there are lots of rows of data)  
poo <- merge(poo, baseline)  
# calculate pupil change from baseline
poo <- mutate(poo, TIMECODE = TIMESTAMP - starttime)
...
# Relabel x gazes that are off the screen as NA (screen dimensions of screen are 1280 x 1024)
poo[, 11][poo[, 11] < 0] <- NA
poo[, 11][poo[, 11] > 1280] <- NA
# Relabel y gazes that are off the screen as NA (screen dimensions of screen are 1280 x 1024)
poo[, 12][poo[, 12] < 0] <- NA
poo[, 12][poo[, 12] > 1024] <- NA
...
#baseline = 1750ms of a trial or 437 samples 
# calculate baseline for every trial
baseline <- poo %>%
  group_by(subID, TRIAL_INDEX) %>%
  summarise(baseline = mean(RIGHT_PUPIL_SIZE[1:436],na.rm = TRUE))

# combine to data base  
poo <- merge(poo, baseline)  

# calculate pupil change from baseline
poo <- mutate(poo, PUPIL_CORRECTED = (RIGHT_PUPIL_SIZE - baseline)/baseline)
...
unique(poo$trackloss)
```

Code every sample if they're looking at target 
```{r}
...
# 1 = target, 2 = distractor, 5 = center, 9 = missing
poo2 <- poo %>%
mutate(lookattarget = ifelse(leftright ==1 & RIGHT_GAZE_X < leftx2, 1,
ifelse(leftright ==1 & RIGHT_GAZE_X > rightx1, 0, 
ifelse(leftright ==2 & RIGHT_GAZE_X > rightx1, 1, 
ifelse(leftright ==2 & RIGHT_GAZE_X < leftx2, 0,
ifelse(is.na(RIGHT_GAZE_X), 9, 5 ))))))

poo2$lookattarget[is.na(poo2$lookattarget)] <- 9

#double check it worked
blah <- subset(poo2, lookattarget==1 & leftright ==1)
...
is.na(poo2$PUPIL_CORRECTED) <- sapply(poo2$PUPIL_CORRECTED, is.infinite)
```

scatterplot of pupil size per subject
```{r}
pupil.sub <- ddply(poo2,.(subID,TRIAL_INDEX),summarise,pupilsize=mean(PUPIL_CORRECTED,na.rm = TRUE))
...
write.csv(poo2, "Pinwheel14_1-17_Infant_Preprocessed.csv", row.names=TRUE) #save to 
```
