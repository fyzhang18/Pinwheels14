...
#import elise data (n=5)
raw <- read.csv("/Volumes/emberson/ResearchProjects/Pupillometry/Pinwheels14/Pinwheel14_elise.csv") 
raw$X <- NULL
```
...
# for the remaining trials remove trackloss (remove NAs)
...
```{r}
#1. Remove trials with pupil change more than 2.5 SDs from the mean
...
for (j in 1:length(subs)) {
  ...
  print(j)
for (i in 1:length(trials)) {
  #if outlier, label 99 in outlier column
